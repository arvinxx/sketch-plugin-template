...
export const File = {
  ...
  _stringify(jsonData, opt) {
    opt = opt ? 1 : 0;
    const nsJson = NSJSONSerialization.dataWithJSONObject_options_error(
      jsonData,
      opt,
      nil
    );
  ...
  copyFile(path, dist) {
    return this.fileManager.fileExistsAtPath(path)
      ? this.fileManager.copyItemAtPath_toPath_error(path, dist, nil)
      : nil;
  },

  mkDir(path) {
    return (
      !this.fileManager.fileExistsAtPath(path) &&
      this.fileManager.createDirectoryAtPath_withIntermediateDirectories_attributes_error_(
        path,
        !0,
        nil,
        nil
  ...
  size(patch) {
    let size = 0;
    try {
      size = this.fileManager.attributesOfItemAtPath_error(patch, nil)
        .NSFileSize;
    } catch (e) {
      size = 0;
    }
  ...
  renameFile(from, to) {
    this.fileManager.fileExistsAtPath(from) &&
      this.fileManager.moveItemAtPath_toPath_error(from, to, null);
  },
  ...
  readDir(path) {
    return (
      !!this.fileManager.fileExistsAtPath(path) &&
      this.fileManager.contentsOfDirectoryAtPath_error_(path, nil)
    );
  },
  ...
  getTempDirPath() {
    ...
    const hash = (Date.now() / 1e3).toString();
