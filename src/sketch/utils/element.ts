...
export const createPanel = (
  ...
  const mainScreenRect = NSScreen.screens().firstObject().frame();

  const Bounds = NSMakeRect(
    options.x
      ? options.x
      : Math.round((NSWidth(mainScreenRect) - options.width) / 2),
    options.y
      ? NSHeight(mainScreenRect) - options.y
      : Math.round((NSHeight(mainScreenRect) - options.height) / 2),
    options.width,
    options.height
  );
